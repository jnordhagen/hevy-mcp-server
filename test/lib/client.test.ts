...
describe("HevyClient - API Contract Compliance", () => {
	...
	// ============================================
	// AGGREGATING PAGINATION HELPERS
	// ============================================
	describe("Aggregating pagination helpers", () => {
		it("should fetch all workout pages and return pagination metadata", async () => {
			mockFetchSuccess(analyticsWorkoutPages[0]);
			mockFetchSuccess(analyticsWorkoutPages[1]);

			const result = await client.getAllWorkouts();
			...
			expect(result.metadata).toEqual({
				...
				truncated: false,
				earlyStopped: false,
				totalItemsBeforeFilter: 4,
				totalItemsAfterFilter: 4,
				since: undefined,
				until: undefined,
				newestFirstOptimizationUsed: false,
			});
			expect(global.fetch).toHaveBeenCalledWith(
				"https://api.hevyapp.com/v1/workouts?page=1&pageSize=10",
				expect.any(Object),
		...
		it("should filter workouts by inclusive date bounds", async () => {
			mockFetchSuccess(analyticsWorkoutPages[0]);
			mockFetchSuccess(analyticsWorkoutPages[1]);
			...
			expect(result.metadata.totalItemsAfterFilter).toBe(2);
		});

		it("should early-stop only after observing newest-first order", async () => {
			mockFetchSuccess({
				...analyticsWorkoutPages[0],
				page_count: 3,
			});
			mockFetchSuccess({
				...analyticsWorkoutPages[1],
				page_count: 3,
			});
			...
			expect(result.metadata.earlyStopped).toBe(true);
			expect(result.metadata.newestFirstOptimizationUsed).toBe(true);
			expect(result.metadata.truncated).toBe(false);
			expect(global.fetch).toHaveBeenCalledTimes(2);
		});

		it("should report truncation when maxPages caps workout paging", async () => {
			mockFetchSuccess({
				...analyticsWorkoutPages[0],
				page_count: 3,
			});

			const result = await client.getAllWorkouts({ maxPages: 1 });

			expect(result.workouts).toHaveLength(2);
			expect(result.metadata.truncated).toBe(true);
			expect(result.metadata.pagesFetched).toBe(1);
			expect(result.metadata.pageCount).toBe(3);
			expect(result.metadata.maxPages).toBe(1);
		});

		it("should fetch all exercise template pages and return metadata", async () => {
			mockFetchSuccess(analyticsTemplatePages[0]);

			const result = await client.getAllExerciseTemplates();

			expect(result.exerciseTemplates).toHaveLength(4);
			expect(result.metadata).toEqual({
				pagesFetched: 1,
				pageCount: 1,
				maxPages: 50,
